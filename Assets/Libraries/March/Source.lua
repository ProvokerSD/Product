...
local SelectedLanguage = Loader_Enabled.Language
...
local old_Visual = CoreGui:FindFirstChild('Visual')

if old_Visual then
    Debris:AddItem(old_Visual, 0)
end
...
local Config = setmetatable({
    save = function(self: any, file_name: any, config: any)
        local success_save, result = pcall(function()
            ...
            writefile('Visual/'..file_name..'.json', flags)
        end)
    ...
    load = function(self: any, file_name: any, config: any)
        local success_load, result = pcall(function()
            if not isfile('Visual/'..file_name..'.json') then
                self:save(file_name, config)
        
                return
            end
        
            local flags = readfile('Visual/'..file_name..'.json')
...
function Library:create_ui()
    ...
    ClientName.Text = getgenv().Get_Name
    ...
    function self:create_tab(title: string, icon: string)
        ...
        function TabManager:create_module(settings: any)
            ...
            if not settings.rich then
                ...
                ModuleName.Text = settings.richtext or "<font color='rgb(255,0,0)'>Visual</font> user"
            end;
            ...
            function ModuleManager:create_paragraph(settings: any)
                ...
                if not settings.rich then
                    ...
                    Body.RichText = true
                    Body.Text = settings.richtext or "<font color='rgb(255,0,0)'>Visual</font> user"
                end
            ...
            function ModuleManager:create_text(settings: any)
                ...
                function TextManager:Set(new_settings)
                    if not new_settings.rich then
                        ...
                        Body.RichText = true
                        Body.Text = new_settings.richtext or "<font color='rgb(255,0,0)'>Visual</font> user"
                    end
                end;
            ...
            function ModuleManager:create_slider(settings: any)
                ...
                local TextLabel = Instance.new('TextLabel')
                if Loader_Enabled.SelectedLanguage == "th" then
